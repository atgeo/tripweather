...
export const add = async (req: TripRequest, res: Response) => {
  try {
    ...
    weatherQueueService.weatherQueue
      .add("fetchWeatherData", { tripId: newTrip._id })
      .then((job: { id: any }) => {
        if (job) {
...
export const update = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const shouldUpdateIsProcessed =
      existingTrip.city !== updatedData.city ||
    ...
    const updatedTrip = await Trip.findByIdAndUpdate(tripId, updateObject, {
      new: true,
    });

    if (updatedTrip && existingTrip.city !== updatedTrip.city) {
      weatherQueueService.weatherQueue
        .add("fetchWeatherData", { tripId })
        .then((job: { id: any }) => {
          if (job) {
            console.log("Job added to queue with ID:", job.id);
          } else {
